...
def safe_para(text, style, max_chars=4000):
    """Paragraph flowable with escaped text; newlines become line breaks."""
    if not text:
        return Paragraph(" ", style)
    s = str(text)
    if len(s) > max_chars:
        s = s[:max_chars] + "… [truncated]"
    return Paragraph(safe(s).replace("\n", "<br/>"), style)
...
def generate_pdf(pdf_filename):
    ...
    pc_tbl = Table(pc_tbl_data, colWidths=pc_col_widths, repeatRows=1, splitInRow=1)
    pc_style_cmds = [
    ...
    if not old_threads_updated:
        ...
        ot_hdr_style = ParagraphStyle("OTHdr2", parent=base_styles["Heading3"],
                                      fontSize=10, spaceBefore=12, spaceAfter=4,
                                      textColor=colors.HexColor("#1a1a6e"))
        ot_meta_style = ParagraphStyle("OTMeta", parent=base_styles["Normal"],
                                       fontSize=8, leading=12, spaceAfter=2)
        ot_label_style = ParagraphStyle("OTLabel", parent=base_styles["Normal"],
                                        fontSize=8, leading=12, fontName="Helvetica-Bold",
                                        textColor=colors.HexColor("#333366"))
        ot_body_style = ParagraphStyle("OTBody", parent=base_styles["Normal"],
                                       fontSize=8, leading=12, spaceAfter=4, leftIndent=8)
        ot_reply_style = ParagraphStyle("OTReply", parent=base_styles["Normal"],
                                        fontSize=8, leading=12, spaceAfter=3, leftIndent=14)
        ...
        for i, thread in enumerate(old_threads_updated, 1):
            ...
            story.append(Paragraph(f"Old Thread #{i}", ot_hdr_style))
            story.append(Paragraph(f"<b>Originally posted:</b> {safe(parent_dt)}", ot_meta_style))
            story.append(Paragraph(f"<b>Author:</b> {safe(parent_user)}", ot_meta_style))
            if permalink:
                story.append(Paragraph(
                    f'<b>Link:</b> <a href="{safe(permalink)}" color="#0000cc">Open in Slack</a>',
                    ot_meta_style))
            story.append(Paragraph("<b>Original message:</b>", ot_label_style))
            story.append(safe_para(parent_text, ot_body_style))

            if earlier_cnt:
                story.append(Paragraph(
                    f"[{earlier_cnt} earlier repl{'y' if earlier_cnt == 1 else 'ies'} "
                    f"before {now.strftime('%Y-%m-%d')} not shown]",
                    ot_meta_style))

            if new_replies:
                story.append(Paragraph(
                    f"<b>New replies on {now.strftime('%Y-%m-%d')} ({len(new_replies)}):</b>",
                    ot_label_style))
                for r in new_replies:
                    ...
                    story.append(safe_para(f"[{r_time}] {r_user}: {r_text}", ot_reply_style))
            else:
                story.append(Paragraph("<i>No new replies on this date.</i>", ot_meta_style))
